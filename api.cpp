...
Options get_options(const std::vector<std::string> &argv)
{
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        if (a == "-v") {
        ...
        } else if (a == "-witness-smtcheck-script") {
            if (++i < argc) {
                ret.witness_smtcheck_script = argv[i];
            } else {
                ok = false;
            }
        ...
        } else if (a == "-inv-reduce") {
            ok = getbool(++i, ret.inv_reduce);
        } else if (a == "-lazy-init-preds") {
            ok = getbool(++i, ret.lazy_init_preds);
        } else if (a == "-abs-bool-vars") {
            ok = getbool(++i, ret.abs_bool_vars);
            ...
            std::cout << "USAGE: " << argv[0] << " [OPTIONS] FILENAME.vmt"
                      ...
                      << "\n   -abs-bool-vars B : include Boolean vars "
                      << "in implicit abstraction"
                      << "\n   -inc-ref B : use incremental refinement"
                      << "\n   -inv-reduce B : enable inductive invariant "
                      << "reduction"
                      << "\n   -lazy-init-preds B : enable lazy initial "
                      << "predicates discovery"
 ...
 * 
 * - ":init" is used to specify the formula for the initial states of the
 ...
 *
 * - ":predicate" is used to annotate a predicate used for computing the
 *   initial abstraction.
 * 
...
bool IC3IA::read_ts(msat_term *terms, char **annots, size_t n)
{
    ...
    logger(1) << "==> init  = " << logterm(env, init) << endlog;
    logger(1) << "==> trans = " << logterm(env, trans) << endlog;
    logger(1) << "==> prop  = " << logterm(env, prop) << endlog;
...
bool IC3IA::check_witness(const std::vector<TermList> &witness)
{
    ...
    if (!opts.witness_smtcheck_script.empty()) {
        FILE *f = fopen(opts.witness_smtcheck_script.c_str(), "w");
        if (f) {
            checker->print_smt_proof_script(f);
            fclose(f);
            logger(1) << "witness SMT check script generated in "
                      << opts.witness_smtcheck_script << endlog;
        } else {
            logger(0) << "ERROR: could not open '"
                      << opts.witness_smtcheck_script
                      << "' for writing" << endlog;
            ok = false;
        }
    }
