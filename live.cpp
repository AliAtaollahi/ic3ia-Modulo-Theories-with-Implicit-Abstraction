...
LiveEncoder::LiveEncoder(TransitionSystem &ts, const Options &opts):
    ...
    statevars_(),
    firstcall_(true),
    klive_counter_(opts, ts.get_env())
{
    ...
    MSAT_MAKE_ERROR_TERM(klive_bound_);
    klive_bound_val_ = 0;
}


void LiveEncoder::initialize()
{
    klive_init_ = msat_make_true(ts_.get_env());
    klive_trans_ = msat_make_true(ts_.get_env());
    set_klive_bound(opts_.live_klive_start);
    
    if (ts_.live_prop() && !opts_.live_klive_only) {
        fairness_ = vp_.fresh_var(".{fair}");
        encode_fairness();

        statevars_.reserve(ts_.statevars().size() + 1);
        for (msat_term v : ts_.statevars()) {
            statevars_.push_back(v);
        }
//        statevars_.push_back(fairness_);

        msat_env env = ts_.get_env();
        delta_ = msat_make_number(env, "1");
        delta_ = update_delta(env, delta_, ts_.prop());
        delta_ = update_delta(
            env, update_delta(env, delta_, ts_.init()), ts_.trans());
    }
}
...
void LiveEncoder::encode(const TermSet &preds, const RankRelList &ranks)
{
    ...
    init_ = msat_make_true(env);
    trans_ = msat_make_true(env);
    ...
    if (opts_.live_klive_only) {
        init_ = klive_init_;
        trans_ = klive_trans_;
        prop_ = klive_counter_.prop();
    } else {
        msat_term fairness = fairness_;
        ...
        msat_term wf = klive_counter_.prop();
        ...
        for (const RankRel &r : ranks) {
            delta_ =
                update_delta(env, update_delta(env, delta_, r.bound), r.rank);
        }
        ...
        for (const RankRel &r : ranks) {
            ...
            msat_term rnkhat =
                msat_apply_substitution(env, r.rank, tosubst.size(),
                                        &tosubst[0], &values[0]);
        ...
        init_ = msat_make_and(env, init_, msat_make_and(env, save, l2s_init_));
        trans_ =
            msat_make_and(
                env, trans_,
                msat_make_and(env, frame,
    ...
    initpreds_.clear();
    get_predicates(env, init_, initpreds_, opts_.abs_bool_vars);
    if (opts_.live_klive_counter) {
        initpreds_.insert(klive_counter_.prop());
    }
...
void LiveEncoder::encode_fairness()
{
    if (opts_.live_klive_only) {
        return;
    }
    ...
    msat_term fair1 = vp_.fresh_var(".{fair}.next");
    ts_.add_statevar(fairness_, fair1);
    //statevars_.push_back(fairness_);
    ...
    msat_term initc =
        msat_make_iff(env, msat_make_not(env, fairness_), prop);
    init_ = msat_make_and(env, init_, initc);
    trans_ = msat_make_and(env, trans_,
                           msat_make_and(env, initc, ts_.next(initc)));
    ts_.add_init(init_);
    ts_.add_trans(trans_);
}
...
{
    msat_term livesignal = ts_.prop();
    if (!opts_.live_klive_only) {
        livesignal = msat_make_not(ts_.get_env(), fairness_);
    }
    ...
    auto enc =
        ...
        {
            sv.clear();
            klive_counter_.get_encoding(init, trans, sv);
            klive_init_ = msat_make_and(ts_.get_env(), klive_init_, init);
            klive_trans_ = msat_make_and(ts_.get_env(), klive_trans_, trans);
            for (auto &p : sv) {
                ts_.add_statevar(p.first, p.second);
                newstatevars_.push_back(p.first);
            }
        };
    
    if (firstcall_) {
        klive_counter_.init(livesignal);
        enc();
        if (opts_.live_klive_only && !klive_bound_val_) {
            // ensure the property holds in the initial states, otherwise
            // the IC3 engine might find "counterexamples" of length 1
            klive_bound_val_ = 1; 
        }
    }
    while (klive_counter_.bound() < klive_bound_val_) {
        klive_counter_.increment();
        enc();
    }
}


void LiveEncoder::set_klive_bound(size_t k)
{
    klive_bound_ = make_number(ts_.get_env(), k);
    klive_bound_val_ = k;
}
...
{
    MSAT_MAKE_ERROR_TERM(origtrans_);
}


void LiveRefiner::initialize()
{
    origtrans_ = ts_.trans();
    statevars_ = ts_.statevars();
                                     ...
                                     const RankRelList &rankrels)
{
    ...
    if (opts_.live_klive_only) {
        ++klive_last_bound_;
        liveenc_.set_klive_bound(klive_last_bound_);
    ...
    assert(ts_.live_prop());
    msat_term fairness = liveenc_.fairness();
    ...
    if ((ok == MSAT_UNDEF || eager_) && use_ranking_) {
        ...
        if (newrank.ok()) {
            ...
            get_predicates(ts_.get_env(), newrank.bound,
                           refine_prefix_preds_, opts_.abs_bool_vars);
    ...
    if (ok == MSAT_UNDEF && klive_progress_) {
        ...
        liveenc_.set_klive_bound(klive_k);
...
KLiveCounter::KLiveCounter(const Options &opts, msat_env env):
    opts_(opts),
    env_(env),
    vp_(env)
{
    MSAT_MAKE_ERROR_TERM(cur_);
...
msat_term KLiveCounter::prop() const
{
    return cur_;
}


void KLiveCounter::init(msat_term prop)
{
    cur_ = prop;
    bound_ = 0;
    init_ = msat_make_true(env_);
    trans_ = msat_make_true(env_);

    logger(2) << "k-liveness counter, property: " << logterm(env_, prop)
              << endlog;

    if (opts_.live_klive_counter) {
        msat_type integer = msat_get_integer_type(env_);
        auto bound = make_number(env_, 0);//opts_.live_klive_start);

        std::string name = "klive.counter";
        msat_term klive_var = vp_.fresh_var(name, integer);
        msat_term klive_var1 = vp_.fresh_var(name + ".next", integer);
        latches_.push_back(std::make_pair(klive_var, klive_var1));

        msat_term fairness = msat_make_not(env_, prop);

        init_ = msat_make_equal(env_, klive_var, msat_make_number(env_, "0"));

        msat_term counter_incr =
            msat_make_equal(env_, klive_var1,
                            msat_make_plus(env_, klive_var,
                                           msat_make_number(env_, "1")));
        msat_term counter_keep = msat_make_equal(env_, klive_var1, klive_var);

        if (opts_.live_klive_nondet) {
            counter_incr = msat_make_or(env_, counter_incr, counter_keep);
        }
            
        trans_ = make_ite(env_, fairness, counter_incr, counter_keep);

        cur_ = msat_make_leq(env_, klive_var, bound);
        kvar_seen_ = false;
    }
}


void KLiveCounter::increment()
{
    ++bound_;

    if (opts_.live_klive_counter) {
        init_ = trans_ = msat_make_true(env_);
        auto bound = make_number(env_, bound_);
        cur_ = msat_make_leq(env_, latches_.back().first, bound);
    } else {
        msat_term sv = vp_.fresh_var("klive.counter");
        msat_term nv = vp_.fresh_var(sv, ".next");
        latches_.push_back(std::make_pair(sv, nv));
        init_ = msat_make_iff(env_, sv, msat_make_true(env_));
        auto update =
            msat_make_iff(env_, nv,
                          make_ite(env_, cur_, sv, msat_make_false(env_)));
        trans_ = update;
        cur_ = msat_make_or(env_, cur_, sv);
    }
}


size_t KLiveCounter::bound() const
{
    return bound_;
}


void KLiveCounter::get_encoding(msat_term &init, msat_term &trans,
                                TermPairList &statevars)
{
    init = init_;
    trans = trans_;
    if (opts_.live_klive_counter) {
        if (!kvar_seen_) {
            ...
            statevars.push_back(latches_.back());
            kvar_seen_ = true;
        }
    } else if (!latches_.empty()) {
        statevars.push_back(latches_.back());
    }
}


TermMap KLiveCounter::get_substitution(size_t counter_value) const
{
    TermMap ret;

    if (opts_.live_klive_counter) {
        ret[latches_.back().first] = make_number(env_, counter_value);
    } else {
        msat_term ff = msat_make_false(env_);
        msat_term tt = msat_make_true(env_);

        size_t i = 0;
        for (; i < std::min(counter_value, latches_.size()); ++i) {
            ret[latches_[i].first] = ff;
        }
        for (; i < latches_.size(); ++i) {
            ret[latches_[i].first] = tt;
        }
    }
